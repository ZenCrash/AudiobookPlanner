...
namespace AudiobookPlanner.API.API.Languages
{
  ...
  public class LanguagesController(ILanguagesManager manager, IStringLocalizer<Localization> localizer) : ControllerBase
  {
    [HttpGet("{id:int}")]
    ...
    public async Task<ActionResult<LanguageDto>> Create(LanguageDto language)
    {
      if (language == null)
        return BadRequest(localizer["Common.AudiobookCannotBeNull"]);
      var result = await manager.CreateAsync(language);
    ...
    [ProducesResponseType(typeof(LanguageDto), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<ActionResult<LanguageDto>> Update(int id, LanguageDto genre)
    {
      if (id == null)
        return BadRequest(localizer["Common.IsCannotBeNull"]);
      if (genre == null)
